...
class UpdateAddressProvider extends ChangeNotifier with ReassembleHandler {
  bool isLoading = true;
  ...
  UpdateAddressProvider(this.context, this.address) {
    // phoneCtrl = address.

    addressCtrl.text = address.address!;
    cityCtrl.text = address.cityName!;
    regionCtrl.text = address.regionName!;
  }
  ...
  getCities(BuildContext context) async {
    ...
    result.fold(
      ...
      (result) async {
        ...
        // }
        if (cityCtrl.text != '') await getRegions(context);
        isLoading = false;
        notifyListeners();
      },
    );
      ...
      (result) {
        ...
        try {
          if (result['message']['title'] == 'موفق') {
            Fluttertoast.showToast(msg: 'آدرس بروز شد');
